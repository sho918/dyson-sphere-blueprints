namespace :mod do
  ...
  task :fetch_base_game_latest, [:patch] => [:environment] do |_t, args|
  ...
  task :flag_breaking, [:name, :patch] => [:environment] do |_t, args|
    mod = Mod.find_by(name: args[:name])
  ...
  task :unflag_breaking, [:name, :patch] => [:environment] do |_t, args|
    mod = Mod.find_by(name: args[:name])
